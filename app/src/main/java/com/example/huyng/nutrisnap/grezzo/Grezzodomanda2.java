...
public class Grezzodomanda2 extends RobotActivity implements RobotLifecycleCallbacks {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.grezzodomanda2);
    }


    //Funzione per il Bottone bianco
    public void Bianco(View view) {
        Intent activity2Intent = new Intent(getApplicationContext(), DomandaG2corretta.class);
        startActivity(activity2Intent); //Per andare alla pagina storie sostanze
    }

    //Funzione per il Bottone scuro
    public void Scuro(View view) {
        Intent activity2Intent = new Intent(getApplicationContext(), DomadnaG2errata.class);
    ...
    public void onRobotFocusGained(QiContext qiContext) {
        ...
        Say sayScelta1 = SayBuilder.with(qiContext)
                .withText("Il riso integrale").build();

        Say sayScelta2 = SayBuilder.with(qiContext)
                .withText(" dona tanta energia e fa bene alla salute?").build();

        Say sayScelta3 = SayBuilder.with(qiContext)
                .withText(" O toglie le energie ed è poco salutare?").build();
...
// Create the phraseSetLearn Talpa.
        PhraseSet phraseSetLearn = PhraseSetBuilder.with(qiContext) // Create the builder using the QiContext.
                .withTexts("dona tanta energia", "prima", "la prima") // Add the phrases Pepper will listen to.
        ...
        PhraseSet phraseSetPlay = PhraseSetBuilder.with(qiContext) // Create the builder using the QiContext.
                .withTexts("toglie le energie", "seconda", "la seconda") // Add the phrases Pepper will listen to.
                .build(); // Build the PhraseSet
